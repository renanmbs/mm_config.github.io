document.addEventListener("DOMContentLoaded", () => {
  ...
  function downloadPDF(filename, svgContent, row) {
    ...
    Promise.all([
      ...
      let property_text = "PROPRIETARY AND CONFIDENTIAL. THE INFORMATION CONTAINED IN THIS DRAWING IS THE SOLE PROPERTY OF MONARCH METAL FABRICATION. ANY REPRODUCTION IN PART OR AS A WHOLE WITHOUT THE WRITTEN PERMISSION OF MONARCH METAL INC. IS PROHIBITED."
      pdf.setFontSize(10);
    ...
    logo.onload = () => {
      ...
      const addDrawings = (index = 0) => {
        ...
        Promise.all([
          ...
          const maxSvgWidth = pageWidth * 0.98;
          const scale = maxSvgWidth / canvas.width;
          const svgWidth = canvas.width * scale;
          const svgHeight = canvas.height * scale;
          ...
          const headers = [
            ...
            "Lead In", "Price Per Piece", "Price For Item", "Weight", "Material", "Finish"
          ];
          const values = [
            row?.["Company Name"] || "",
            row?.Quantity || "",
          ...
          // --- Notes ---
          doc.setFontSize(25);
          doc.text("Notes:", margin, noteY, { align: "left" });
