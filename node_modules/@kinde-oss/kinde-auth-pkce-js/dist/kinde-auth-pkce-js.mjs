...
const Xr = 60, Ur = (u) => {
  ...
  const l = f || "spa@live", g = location.hostname === "localhost" || location.hostname === "127.0.0.1", E = !g && !d && ie(p), A = g || d, B = {
    ...
    const x = Q("org_code");
    return {
      isGranted: (Q("permissions") ?? []).some((I) => I === h),
      orgCode: x
    };
  }, yr = () => ({
    orgCode: Q("org_code")
  }), sr = () => {
    const h = new URL(window.location.toString());
    h.search = "", window.history.pushState({}, "", h);
  }, gr = async (h) => {
    const x = h.get("form_data");
    if (x) {
      const M = JSON.parse(
        Wr.Buffer.from(x, "base64").toString("utf-8")
      );
      rr({ authUrlParams: M });
      return;
    }
    const L = h.get("code"), I = h.get("state"), O = h.get("error"), j = sessionStorage.getItem(`${tr}-${I}`);
    if (!j)
      console.error("Invalid state");
    else {
      if (O) {
        const W = h.get("error"), Y = h.get("error_description");
        sr(), sessionStorage.removeItem(`${tr}-${I}`);
        const { appState: q } = JSON.parse(j);
        return D ? D({
          error: W,
          errorDescription: Y,
          state: I,
          appState: q
        }) : window.location.href = q.kindeOriginUrl, !1;
      }
      const { appState: M, codeVerifier: X } = JSON.parse(j);
      try {
        const Y = await (await fetch(B.token_endpoint, {
          method: "POST",
          ...E && { credentials: "include" },
          headers: new Headers({
            "Content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            "Kinde-SDK": `${B._framework || "JavaScript"}/${B._frameworkVersion || kr}`
          }),
          body: new URLSearchParams({
            client_id: B.client_id,
            code: L,
        ...
        k(Y), sr(), sessionStorage.removeItem(`${tr}-${I}`);
        const q = ur();
        o && o(q, M);
      } catch (W) {
        console.error(W), sessionStorage.removeItem(`${tr}-${I}`);
      }
    }
  }, v = async (h) => {
    const {
    ...
    x.kindeOriginUrl || (x.kindeOriginUrl = window.location.href);
    const { state: X, code_challenge: W, url: Y } = await Zr(
      ...
      client_id: l,
      response_type: "code",
      scope: B.requested_scopes,
      code_challenge: W,
      code_challenge_method: "S256",
      state: X
    };
    L && (q.prompt = L), j && (q.org_code = j), I && (q.is_create_org = String(I), q.org_name = O);
    const z = new URLSearchParams(
      Object.assign(M, q)
    );
    a && a.trim().split(/\s+/).forEach((Z) => {
      z.append("audience", Z);
    }), Y.search = String(z), window.location.href = Y.toString();
  }, cr = async (h) => {
    await v({
      ...h,
      prompt: "create"
  ...
  }, ur = () => J.getItem(P.user), xr = async () => {
    const x = {
      Accept: "application/json",
      Authorization: `Bearer ${await U()}`
    };
    try {
      const I = await (await fetch(`${B.domain}/oauth2/v2/user_profile`, {
        method: "GET",
        headers: x
      })).json();
      return J.setItem(P.user, {
        id: I.sub,
    ...
    const h = new URL(`${B.domain}/logout`);
    try {
      J.reset(), A && localStorage.removeItem(P.refresh_token);
      const x = new URLSearchParams({
        redirect: w
      });
      h.search = String(x), window.location.href = h.toString();
    } catch (x) {
      console.error(x);
    }
  }, Br = async () => {
    const h = new URLSearchParams(window.location.search);
    $(h) ? await gr(h) : (E || A) && await b();
  }, $ = (h) => {
    const x = h.has("code"), L = h.has("error");
    if (!x && !L) return !1;
    const { protocol: I, host: O, pathname: j } = window.location, M = R || `${I}//${O}${j}`;
    return M === y || M === `${y}/`;
  };
  return await Br(), {
    getToken: U,
